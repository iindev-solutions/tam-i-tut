...
| 4 | Content & launch: pilot seed, freshness SLA, closed pilot | IN PROGRESS (content: 29 places, guides across 8 categories; trust levels surface in the UI; emergency contacts verified + completed. Next: hospital directory needs field verification, then closed pilot + metrics) |
...
## Notes
- 2026-09-15 (6): admin coverage closed - `/admin/clinics` and `/admin/consulates` editors added (the capability migration 062 existed for), write path proven by data + audit. Recorded a schema trap for the founder: a staff account becomes undeletable after its first audited action (FK + append-only trigger), fix would be `on delete set null` on `audit_logs.actor_profile_id`. Details: vault/logs/changelog.md.
- 2026-09-15 (5): review pass - removed an outage risk from the clinics slice (secondary reads now degrade per-table instead of blanking the app), moved consular facts into a `consulates` table so they are admin-correctable, fixed a false provenance claim in the clinics copy, and corrected a pgTAP assertion about RLS-denied UPDATE. Details: vault/logs/changelog.md.
- 2026-09-15 (4): Da Nang clinic directory shipped (migration 061, pgTAP 017) - 20 venues with prices on `/categories/health`, all `under_review` with the source printed above the list, enforced by the DB trust-date gate. Read-only in the UI for now (no `/admin` editor). Details: vault/logs/changelog.md.
- 2026-09-15 (3): safety page corrected and completed - consulate address was wrong (Trần Hưng Đạo -> 22 Trần Phú, verified against the official MFA site) plus its 24/7 line, emergency numbers gained 112 and the Da Nang tourism line (migration 060), and every number is now a `tel:` link. Competitor research (vietnamspot.ru) recorded in resume-plan as a ranked gap list. Details: vault/logs/changelog.md.
