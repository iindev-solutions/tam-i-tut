...
- Trust is `TrustBadge.vue`: three levels from the DB enum (`under_review` / `recommended_expats` / `verified_team`) plus the check date. Orange marks `verified_team` only - it is the one trust cue per screen; lower levels stay `text-muted`/`text-dimmed` so a card list never turns into color noise. Always use the `full` variant on user surfaces - the `icon` variant conveys its meaning only through `title`/`aria-label`, and `title` tooltips do not exist on touch, which is the entire TMA. The `icon` variant is for the admin table, where a pointer is present. Copy: the level names the trust and the date only needs to be a date, so a trusted entry reads "Проверено командой · 30.08.2026" / "Verified by the team · 30/08/2026" - never repeat the verb. The DB enforces the pairing - a trusted row cannot be saved without `last_verified_at`. Never render `under_review_note` to users: it is internal editorial process text (seeded values read like "Контент из knowledge-dump, ждёт проверки").
