...
- Admin coverage (2026-09-15 (6)): `/admin/clinics` and `/admin/consulates` editors - the capability migration 062 was created for. Write path proven by two real saves + two audit rows. Found and recorded a schema trap: a staff account becomes UNDELETABLE after its first audited action (`audit_logs.actor_profile_id` FK + append-only trigger), so a departing or compromised admin can only be neutralized; proposed fix is `on delete set null` on that FK (founder decision, touches a trust-critical table). Also fixed malformed markup in `/admin/cities` and an accessible-name bug where `aria-label` masked the row button's visible action text.
